...
async function loaded(){await page.evaluate(async()=>{await document.fonts.ready;document.documentElement.style.scrollBehavior='auto';for(let y=0;y<document.body.scrollHeight;y+=450){scrollTo(0,y);await new Promise(r=>setTimeout(r,45))}});for(const item of await page.locator('.reveal').all()){if(await item.evaluate(el=>el.classList.contains('is-pending'))){await item.scrollIntoViewIfNeeded();await expect(item).not.toHaveClass(/is-pending/)}}await page.evaluate(()=>scrollTo(0,0));await page.waitForTimeout(500)}
